...
pub struct GenerateProofError(pub String);

impl fmt::Display for GenerateProofError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.0)
    }
}

impl error::Error for GenerateProofError {}
...
pub enum ZError {
    ...
    IO(io::Error),
    Mq(mq::Error),
    GenerateProof(GenerateProofError),
}
...
impl fmt::Display for ZError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            ZError::GenerateProof(e) => write!(f, "{}", e),
        }
    }
}
...
impl error::Error for ZError {
    fn source(&self) -> Option<&(dyn error::Error + 'static)> {
        match self {
            ...
            ZError::GenerateProof(ref e) => Some(e),
        }
    }
}
