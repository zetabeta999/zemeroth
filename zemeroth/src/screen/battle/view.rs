...
use battle::{
    battle::{
        ability::Ability, check as command_check, command, component::ObjType, execute::hit_chance,
        movement, Id, Jokers, Moves, State, TileType, Turns,
    },
    map::{self, Dir, HexMap, PosHex},
};
...
impl BattleView {
    pub fn new(map_radius: i32) -> ZResult<Self> {
    ...
    pub fn update_disappearing_sprites(&mut self) -> Box<dyn Action> {
        ...
        sprites.disappearing_sprites.retain(|s| s.turns_left.0 > 0);
    ...
    fn select_ability(&mut self, state: &State, selected_id: Id, ability: Ability) -> ZResult {
        ...
        for pos in positions {
            ...
            if command_check(state, &command).is_ok() {
    ...
    fn show_selection_marker(&mut self, state: &State, id: Id) {
        let pos = state.parts().pos.get(&id).unwrap().0;
        let point = hex_to_point(self.tile_size(), pos);
    ...
    fn show_attackable_tiles(&mut self, state: &State, id: Id) -> ZResult {
        ...
        let selected_agent_player_id = parts.belongs_to.get(&id).unwrap().0;
        for target_id in parts.agent.keys() {
            let target_pos = parts.pos.get(target_id).unwrap().0;
            let target_player_id = parts.belongs_to.get(target_id).unwrap().0;
            ...
            .into();
            if command_check(state, &command_attack).is_err() {
                continue;
            }
            self.show_hit_chance_label(state, id, *target_id)?;
            self.highlight_tile(target_pos, TILE_COLOR_ATTACKABLE)?;
        }
        Ok(())
    }
    ...
    fn show_walkable_tiles(
        ...
        let agent = state.parts().agent.get(&id).unwrap();
        ...
        for pos in map.iter() {
            ...
            self.highlight_tile(pos, TILE_COLOR_WALKABLE)?
        }
        Ok(())
    }
    ...
    fn show_hit_chance_label(&mut self, state: &State, attacker_id: Id, target_id: Id) -> ZResult {
        let target_pos = state.parts().pos.get(&target_id).unwrap().0;
...
pub fn make_action_create_map(state: &State, view: &BattleView) -> ZResult<Box<dyn Action>> {
    ...
    for hex_pos in state.map().iter() {
        ...
        // if is_free && is_plain && roll_dice_system(0, 10) < 2 {
